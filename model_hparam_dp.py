...
#    print(df.isnull().sum())
#    print(df.describe())
    print('C1.3', time.ctime())
    neg, pos = np.bincount(df[target_col])

    print('C1.4', time.ctime())
...
def build_mod(metrics:list, encoded_features:list, all_inputs:list, optimizer:str='adam' ,output_bias=None, opt_list:list=None):
    ...
    dp_optimizer = tensorflow_privacy.DPKerasSGDOptimizer(
        l2_norm_clip=opt_list[0],
        noise_multiplier=opt_list[1],
        num_microbatches=opt_list[2],
        learning_rate=opt_list[3])
...
def run(opt_list, train_ds, val_ds, test_ds):
    ...
    model = build_mod(metrics=metrics, encoded_features=encoded_features, all_inputs=all_inputs, output_bias=init_bias, opt_list=opt_list)
    ...
    f.write("l2_norm_clip: " + str(opt_list[0]) + "\n")
    f.write("noise_multiplier: " + str(opt_list[1]) + "\n")
    f.write("num_microbatches: " + str(opt_list[2]) + "\n")
    f.write("learning_rate: " + str(opt_list[3]) + "\n")
    f.write(compute_dp_sgd_privacy.compute_dp_sgd_privacy_statement(number_of_examples=67843,
                                              batch_size=256,
                                              noise_multiplier=opt_list[1],
                                              num_epochs=10,
                                              delta=1e-5))
...
if __name__=='__main__':
    ...
    for l2_norm in np.arange(0.2, 1.0, 0.2):
        for noise_mult in np.arange(1.0, 2.0, 0.2):
            for num_micro in [1]:
                for learn_rate in np.arange(0.1, 0.7, 0.1):
                    opt_list = [l2_norm, noise_mult, num_micro, learn_rate]
                print("starting", opt_list)
                run(opt_list,train_ds,val_ds,test_ds)
